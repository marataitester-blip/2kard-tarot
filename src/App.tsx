...
const App: React.FC = () => {
  ...
  const checkAccess = (targetConsultant: ConsultantType, targetMode: AppMode): boolean => {
    if (targetConsultant === 'VIP') return targetMode === 'BLITZ';
    if (targetConsultant === 'STANDARD') return ['BLITZ', 'RELATIONSHIPS', 'FATE'].includes(targetMode);
    return true;
  };
  ...
  const RenderLayout = () => {
    if (appMode === 'BLITZ') return <div className="w-48 aspect-[2/3] mx-auto"><CardImage card={selectedCards[0]} /></div>;
    if (appMode === 'RELATIONSHIPS') return <div className="flex justify-center gap-4 h-full items-center"><div className="w-[45%] max-w-[180px] aspect-[2/3]"><CardImage card={selectedCards[0]} /></div><div className="w-[45%] max-w-[180px] aspect-[2/3]"><CardImage card={selectedCards[1]} /></div></div>;
    if (appMode === 'FATE') return <div className="flex justify-center gap-2 h-full items-center"><div className="w-[32%] aspect-[2/3]"><CardImage card={selectedCards[0]} /></div><div className="w-[32%] aspect-[2/3]"><CardImage card={selectedCards[1]} /></div><div className="w-[32%] aspect-[2/3]"><CardImage card={selectedCards[2]} /></div></div>;
    if (appMode === 'FINANCE') return <div className="grid grid-cols-2 gap-4 max-w-[300px] mx-auto aspect-square"><CardImage card={selectedCards[0]} /><CardImage card={selectedCards[1]} /><CardImage card={selectedCards[2]} /><CardImage card={selectedCards[3]} /></div>;
            ...
            {introStep === 'LAYOUT' && (
              <div className="absolute inset-0 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
                 <div className="w-full max-w-sm bg-[#0a0a0a]/90 border border-white/10 p-6 rounded-2xl shadow-2xl flex flex-col gap-3">
                    ...
                    <button onClick={() => handleLayoutSelect('BLITZ')} className="w-full py-3 bg-white/5 border border-white/10 hover:border-[#D4AF37] rounded flex justify-between px-4 items-center group"><span className="text-gray-300 text-xs uppercase font-bold tracking-widest group-hover:text-[#D4AF37]">⚡ Блиц</span><span className="text-[9px] bg-[#333] px-2 py-0.5 rounded text-gray-400">1 карта</span></button>
                    <button onClick={() => handleLayoutSelect('RELATIONSHIPS')} className="w-full py-3 bg-white/5 border border-white/10 hover:border-[#D4AF37] rounded flex justify-between px-4 items-center group"><span className="text-gray-300 text-xs uppercase font-bold tracking-widest group-hover:text-[#D4AF37]">❤️ Отношения</span><span className="text-[9px] bg-[#333] px-2 py-0.5 rounded text-gray-400">2 карты</span></button>
                    <button onClick={() => handleLayoutSelect('FATE')} className="w-full py-3 bg-white/5 border border-white/10 hover:border-[#D4AF37] rounded flex justify-between px-4 items-center group"><span className="text-gray-300 text-xs uppercase font-bold tracking-widest group-hover:text-[#D4AF37]">🔮 Судьба</span><span className="text-[9px] bg-[#333] px-2 py-0.5 rounded text-gray-400">3 карты</span></button>
                    <button onClick={() => handleLayoutSelect('FINANCE')} className="w-full py-3 bg-white/5 border border-white/10 hover:border-[#D4AF37] rounded flex justify-between px-4 items-center group"><span className="text-gray-300 text-xs uppercase font-bold tracking-widest group-hover:text-[#D4AF37]">💸 Финансы</span><span className="text-[9px] bg-[#D4AF37] text-black px-2 py-0.5 rounded font-bold">PRO</span></button>
                    <button onClick={() => handleLayoutSelect('CROSS')} className="w-full py-3 bg-white/5 border border-white/10 hover:border-[#D4AF37] rounded flex justify-between px-4 items-center group"><span className="text-gray-300 text-xs uppercase font-bold tracking-widest group-hover:text-[#D4AF37]">✝️ Крест</span><span className="text-[9px] bg-[#D4AF37] text-black px-2 py-0.5 rounded font-bold">PRO</span></button>
            ...
            {introStep === 'INPUT' && (
              <div className="absolute inset-0 flex flex-col pt-20 px-4 bg-black/70 backdrop-blur-md animate-fade-in items-center">
                 <div className="w-full max-w-md bg-[#050505] border border-[#D4AF37]/30 p-6 rounded-xl shadow-2xl relative">
                    ...
                    <textarea value={userProblem} onChange={(e) => setUserProblem(e.target.value)} placeholder={consultant === 'VIP' ? "Излагайте суть..." : "Что случилось?"} autoFocus className="w-full h-32 bg-transparent border-b border-[#333] text-lg text-gray-200 focus:border-[#D4AF37] outline-none resize-none font-serif placeholder-gray-600 mb-6"/>
                    <button onClick={handleStartSession} disabled={!userProblem.trim()} className="w-full py-4 bg-[#D4AF37] text-black font-bold uppercase tracking-[0.2em] rounded hover:bg-[#FFD700] transition-transform active:scale-95 disabled:opacity-50">Начать Сеанс</button>
        ...
        {screen === 'OFFICE' && (
          <div className="w-full h-screen flex flex-col p-4 animate-fade-in">
            <div className="w-full flex justify-between items-center mb-4 z-20">
               <button onClick={fullReset} className="text-[10px] text-gray-400 hover:text-[#D4AF37] flex items-center gap-1 uppercase tracking-widest bg-black/40 px-3 py-1 rounded backdrop-blur-sm"><span>✕</span> Выход</button>
            </div>

            <div className="flex-grow flex flex-col relative overflow-hidden">
               {/* 1. ЗОНА КАРТ */}
               <div className={`transition-all duration-700 w-full flex flex-col items-center justify-center ${analysisStep === 'TABLE' ? 'flex-grow h-full' : 'h-[35vh] min-h-[220px] shrink-0'}`}>
                  ...
                  <div className="flex-grow bg-[#050505]/90 backdrop-blur-xl border-t border-[#D4AF37]/30 rounded-t-2xl shadow-[0_-10px_40px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden animate-slide-up">
                     <div className="p-4 border-b border-[#333] flex items-center justify-between shrink-0">
                        <div className="flex items-center gap-2">
                           ...
                           <span className={`text-xs font-bold uppercase tracking-widest ${consultant === 'VIP' ? 'text-[#FFD700]' : 'text-[#D4AF37]'}`}>{consultant === 'VIP' ? 'МЕССИР' : 'МАРГО'}</span>
                        </div>
                        <div className="flex gap-4">
                           <button onClick={handleCopyText} className="text-gray-400 hover:text-white" title="Копировать текст">📋</button>
                           <button onClick={handleDownloadImage} className="text-gray-400 hover:text-white" title="Скачать фото">📸</button>
                           <button onClick={handleShare} className="text-gray-400 hover:text-white" title="Поделиться">🔗</button>
                           {!audioUrl ? (<button onClick={handleGenerateAudio} disabled={isGeneratingVoice} className="text-gray-400 hover:text-white">{isGeneratingVoice ? '⏳' : '🔊'}</button>) : (<audio controls src={audioUrl} className="h-6 w-20 opacity-70" autoPlay />)}
                        </div>
                     </div>
