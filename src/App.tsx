...
const App: React.FC = () => {
  ...
  const [zoomedCard, setZoomedCard] = useState<TarotCard | null>(null); // Для увеличения
  ...
  const CardImage = ({ card }: { card: TarotCard | null }) => {
    ...
    return (
      <div 
        className="w-full h-full relative animate-flip-in cursor-zoom-in"
        onClick={() => setZoomedCard(card)} // Клик для увеличения
      >
  ...
  return (
    <div className="min-h-screen font-serif flex flex-col relative overflow-hidden text-[#E0E0E0] selection:bg-[#D4AF37] selection:text-black">
      ...
      {/* МОДАЛКА ЗУМА (УВЕЛИЧЕНИЕ КАРТЫ) */}
      {zoomedCard && (
        <div 
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-6 animate-fade-in cursor-zoom-out"
          onClick={() => setZoomedCard(null)}
        >
          <div className="relative max-w-lg w-full max-h-[80vh] aspect-[2/3]">
             ...
             <div className="absolute bottom-[-40px] left-0 w-full text-center text-[#D4AF37] text-lg font-cinzel font-bold">
               {zoomedCard.name}
             </div>
          </div>
        </div>
