...
const App: React.FC = () => {
  ...
  const runDiagnosis = async (forcedConsultant?: ConsultantType) => {
    ...
    try {
      const text = await analyzeRelationship(
  ...
  const CardImage = ({ card }: { card: TarotCard | null }) => {
    if (!cardsRevealed) return (
      <img src={ASSETS.img_cardback} className="w-full h-full object-cover rounded shadow-lg animate-pulse" alt="Cover" />
    );
    return (
      <div className="w-full h-full relative animate-flip-in cursor-zoom-in group" onClick={() => setZoomedCard(card)}>
        <img 
          src={card?.imageUrl} 
          className="w-full h-full object-cover rounded shadow-lg transition-transform group-hover:scale-105" 
          alt={card?.name} 
          ...
          {introStep === 'LAYOUT' && (
             <div className="absolute inset-0 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm z-50">
                <div className="w-full max-w-sm bg-[#111] border border-white/10 p-6 rounded-2xl shadow-2xl flex flex-col gap-3">
                   ...
                   <button onClick={() => handleLayoutSelect('BLITZ')} className="w-full py-3 bg-white/5 border border-white/10 hover:border-[#D4AF37] rounded flex justify-between px-4 items-center"><span className="text-xs uppercase font-bold text-gray-300">⚡ Блиц</span><span className="text-[9px] text-gray-500">1 карта</span></button>
                   ...
                   <button onClick={() => handleLayoutSelect('FATE')} className="w-full py-3 bg-white/5 border border-white/10 hover:border-[#D4AF37] rounded flex justify-between px-4 items-center"><span className="text-xs uppercase font-bold text-gray-300">🔮 Судьба</span><span className="text-[9px] text-gray-500">3 карты</span></button>
