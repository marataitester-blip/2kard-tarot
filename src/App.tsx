...
// АССЕТЫ (версии v=2 для сброса кэша)
const ASSETS = {
    ...
    if (appMode === 'RELATIONSHIPS') {
      return (
        <div className="flex justify-center gap-4 h-full items-center">
          <div className="w-[45%] max-w-[180px] aspect-[2/3]"><CardImage card={selectedCards[0]} /></div>
          <div className="w-[45%] max-w-[180px] aspect-[2/3]"><CardImage card={selectedCards[1]} /></div>
        </div>
      );
    }
    if (appMode === 'FATE') {
      return (
        <div className="flex justify-center gap-2 h-full items-center">
          <div className="w-[32%] aspect-[2/3]"><CardImage card={selectedCards[0]} /></div>
          <div className="w-[32%] aspect-[2/3]"><CardImage card={selectedCards[1]} /></div>
          <div className="w-[32%] aspect-[2/3]"><CardImage card={selectedCards[2]} /></div>
        </div>
      );
    }
    if (appMode === 'FINANCE') {
    ...
    <div className="min-h-screen font-serif flex flex-col relative overflow-hidden text-[#E0E0E0] selection:bg-[#D4AF37] selection:text-black">
      ...
      {showPaywall && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-6 animate-fade-in">
           <div className="bg-[#111] border border-[#D4AF37] p-8 rounded-xl max-w-sm w-full text-center relative shadow-lg">
                  ...
                  <div className="flex-grow bg-[#050505]/90 backdrop-blur-xl border-t border-[#D4AF37]/30 rounded-t-2xl shadow-[0_-10px_40px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden animate-slide-up">
                     ...
                     <div className="p-4 border-b border-[#333] flex items-center justify-between shrink-0">
                        ...
                        <div className="flex gap-4">
                           ...
                           {!audioUrl ? (
                             <button onClick={handleGenerateAudio} disabled={isGeneratingVoice} className="text-gray-400 hover:text-white">
                               {isGeneratingVoice ? '⏳' : '🔊'}
                             </button>
                           ) : (
                             <audio controls src={audioUrl} className="h-6 w-20 opacity-70" autoPlay />
                           )}
                        </div>
                     </div>
